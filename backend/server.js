...
const fileUpload = require('express-fileupload'); // Importação correta
const fs = require('fs');

// Inicialização do Express
const app = express(); // Certifique-se de que `app` seja inicializado aqui
app.use(cors());
app.use(bodyParser.json());
app.use(fileUpload()); // Agora pode ser usado sem erros
...
app.post('/evaluations/avaliacao-feminina', (req, res) => {
    ...
    const dataAtual = new Date().toISOString(); // Formato de data ISO 8601
...
// Endpoint para realizar o backup do banco de dados
app.get('/api/backup', (req, res) => {
    const filePath = path.join(__dirname, 'database.db');
    
    // Verifica se o arquivo do banco de dados existe
    if (!fs.existsSync(filePath)) {
        return res.status(404).send('Banco de dados não encontrado.');
    }
...
// Endpoint para restaurar o banco de dados
app.post('/api/restore', (req, res) => {
    ...
    // Substituir o arquivo do banco de dados atual pelo arquivo enviado
    uploadedFile.mv(filePath, (err) => {
        if (err) {
            console.error('Erro ao restaurar o banco de dados:', err);
            return res.status(500).send('Erro ao restaurar o banco de dados.');
        }

        res.status(200).send('Banco de dados restaurado com sucesso.');
    });
});
