...
const Students = () => {
    ...
    const handleAddEvaluation = (student) => {
        const currentDate = new Date().toLocaleDateString(); // Obtém a data atual no formato "dd/mm/yyyy"
        const evaluationData = {
            alunoId: student.id,
            nome: student.name,
            idade: student.age,
            data: currentDate,  // Adiciona a data da avaliação
        };
    
        if (student.gender === 'Masculino') {
            navigate('/avaliacao-masculina', { state: evaluationData });
        } else if (student.gender === 'Feminino') {
            navigate('/avaliacao-feminina', { state: evaluationData });
        }
    };
    ...
    const generateEvaluationsPDF = async (student) => {
        ...
        const doc = new jsPDF('landscape'); // Altera para orientação paisagem
        ...
        if (evaluations.length === 0) {
            ...
            // Adiciona a tabela de avaliações
            doc.autoTable({
                startY: 35,
            ...
            // Adiciona comparativo das últimas avaliações
            ...
            if (comparison) {
                ...
                doc.autoTable({
                    ...
                    headStyles: { fillColor: [78, 204, 196] },
                    columnStyles: {
                        3: { halign: 'right' } // Alinha a coluna de diferença à direita
                    }
                });
            }
        }
